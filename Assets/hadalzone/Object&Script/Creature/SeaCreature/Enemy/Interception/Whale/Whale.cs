...
class Whale : Interception
{
    ...
    private void Update()
    {
        ...
        if (alive == true)
        {
            ...
            {
                NormalMove();
                if (playerVisibleSpace == true || playerAttackPersonalSpace == true)
                {//Playerが視界範囲、またはPlayerの攻撃が個体範囲に入れば警戒状態に
                    cautionState = true;
                    recognizeState = false;
                }
                if (coreTouch == true)
            ...
            if (cautionState == true)
            {
                ...
                {//Playerが一定時間攻撃しなければNormalMoveを行う
                    NormalMove();
                }
                if (RemaindCautionCount <= RemaindCautionTime)
                {
                ...
                if (playerPersonalSpace == true || playerAttackVisibleSpace == true)
                {//Playerが個体範囲、またはPlayerの攻撃が視界範囲に入れば敵対状態に
                    hostileState = true;
                    cautionState = false;
                    RemaindCautionCount = 0;
                }
                if (playerRecognizeSpace == false)
                {//Playerが認識範囲から出たなら警戒解除
                    cautionState = false;
                    RemaindCautionCount = 0;
                }
            }
            ...
            {
                Intercept();
                if (playerVisibleSpace == false && playerAttackVisibleSpace == false)
                {//PlayerとPlayerの攻撃が視覚範囲に入らない状態が一定時間続くと敵対状態が解除され認識状態に移行する
    ...
    public override void Intercept()
    {
        ...
        SKILL.iceExplodeCoolTimeCount += Time.deltaTime;//弾の発射
        if (SKILL.iceExplodeCoolTimeCount > SKILL.iceExplodeIntervalTime * offensiveSpeed)
        {
            SKILL.iceExplode(offensivePower, offensiveRange, creaturetype, myangle.z + 100, preyMouth.MouthPosition, myrotation);
        }
